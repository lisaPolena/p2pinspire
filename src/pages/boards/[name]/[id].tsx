...
export default function DetailBoard() {
  ...
  useContractEvent({
    ...
    listener(log: any) {
      ...
      const newBoard = {
        ...
        owner: args.owner,
        pins: args.pins,
        boardCoverHash: args.boardCoverHash,
      } as Board;
      setBoard(newBoard);
      onBoardEdited(
        Number(args.boardId),
  ...
  function getPinsByBoardId() {
    if (boardById && board && allPins) {
      ...
      const boardPins = tmpPins.filter((pin: Pin) =>
  ...
  const onPinCreated = (
    ...
    setPins((prevPins) => {
      return [
        ...prevPins.filter(({ id }) => Number(id) !== Number(boardId)),
        newPin,
    ...
    <>
      ...
      <main className="min-h-screen overflow-auto bg-black mb-18">
        ...
        {board ? (
          <div className="relative top-[50px]">
            ...
            {!isLoading ? (
              <>
                <div>
                  <p className="text-[1.2rem] ml-[1.3rem] mb-[0.4rem] font-semibold">
        ...
        <button
          ...
          className="flex fixed w-[50px] h-[50px] rounded-full bottom-5 left-[45vw] bg-secondary items-center justify-center"
        >
